...
const InfoSection = () => {
    ...
    return(
    info.map((item, index)=>(
        <div key={index} className='p-4 flex flex-col gap-3.5 h-1/2 w-full sm:w-full md:w-[45vw] rounded-md shrink-0 text-white bg-zinc-950'>
            <h1 className='text-6xl text-center font-[inknut]' key={index}>{item.title}</h1>
            <p className='text-2xl text-center font-[inknut]' key={index}>{item.para}</p>
        </div>
    ))
  )
}
